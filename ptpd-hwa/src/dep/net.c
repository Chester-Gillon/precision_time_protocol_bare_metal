                     ...
                     Octet *uuid, NetPath *netPath)
{
  ...
  device[i].ifr_data = (void *)&hwconfig;
  DBG("Setting the time stamping tx: %d, rx: %d\n",hwconfig.tx_type,hwconfig.rx_filter);

  if (ioctl(netPath->eventSock, SIOCSHWTSTAMP, &device[i]) < 0)
    {
      if (errno == ERANGE)
        {
...
{
  int temp, i, so_timestamping_flags;
  ...
  *(Integer16*)ptpClock->general_port_address = PTP_GENERAL_PORT;
  ...
  if (netPath->HWAccel)
    ...
    so_timestamping_flags = SOF_TIMESTAMPING_RX_SOFTWARE | SOF_TIMESTAMPING_TX_SOFTWARE | SOF_TIMESTAMPING_SOFTWARE;
...
Boolean netShutdown(NetPath *netPath)
{
  ...
  if (!netPath->init && netPath->HWAccel)
    {
      struct hwtstamp_config hwconfig;
...
ssize_t netRecvEvent(Octet *buf, TimeInternal *time, NetPath *netPath, Boolean areWeMaster)
{
  ssize_t ret = 0;
  struct msghdr msg;
  struct iovec vec[1];
  ...
  {
    struct cmsghdr cm;
    //char control[CMSG_SPACE(sizeof(struct scm_timestamping)+sizeof(struct sock_extended_err))];
    char control[512];
  } cmsg_un;
  ...
  if(netPath->HWAccel)
  {
	  ret = recvmsg(netPath->eventSock, &msg, MSG_ERRQUEUE|MSG_DONTWAIT);
  	rs="send";
  ...
  if (ret <= 0)
  ...
  else
    {
       ...
       */
      DBGV("Received %d bytes\n", ret);
      DBGV("Last sent event message size: %d bytes\n",netPath->lastNetSendEventLength);
      if (ret > netPath->lastNetSendEventLength)
        {
          memmove(buf, buf + ret - netPath->lastNetSendEventLength, netPath->lastNetSendEventLength);
          ret = netPath->lastNetSendEventLength;
        }
      else
        {
      ...
      if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SO_TIMESTAMPING)
        {
          ...
          if (netPath->HWAccel) stamp++;
        }
    }

  if (stamp && stamp->tv_sec && stamp->tv_nsec)
    {
      time->seconds = stamp->tv_sec;
      time->nanoseconds = stamp->tv_nsec;
      DBG("kern %s time stamp %us %dns\n", rs, time->seconds, time->nanoseconds);
    }
  else
    {
      ...
      DBGV("no event recieve time stamp\n");
      return 0;
    }
...
ssize_t netSendEvent(Octet *buf, UInteger16 length, NetPath *netPath)
{
  ...
  netPath->lastNetSendEventLength = length;
...
static Boolean selectNICTimeMode(Boolean sync, NetPath *netPath)
{
...
{
  if(ptpClock->netPath.HWAccel && state != ptpClock->port_state)
    {
      if(state == PTP_MASTER)
        /* only master listens for Delay_Req... */
        selectNICTimeMode(FALSE, &ptpClock->netPath);
      else if(ptpClock->port_state == PTP_MASTER)
        /** ... and only while he still is master */
        selectNICTimeMode(TRUE, &ptpClock->netPath);
    }
}
