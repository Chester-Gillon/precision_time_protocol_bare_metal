...
Boolean doInit(RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
  ...
  ptpClock->netPath.HWAccel = rtOpts->HWAccel;
  ptpClock->netPath.init = TRUE;
  netShutdown(&ptpClock->netPath);
  ptpClock->netPath.init = FALSE;
...
void handle(RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
  ...
  length = netRecvEvent(ptpClock->msgIbuf, &time, &ptpClock->netPath, ptpClock->port_state == PTP_MASTER);
  if (length < 0)
    {
